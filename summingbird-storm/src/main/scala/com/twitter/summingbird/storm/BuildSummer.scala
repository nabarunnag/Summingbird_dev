...
object BuildSummer {
  ...
  def apply(storm: Storm, dag: Dag[Storm], node: StormNode, jobID: JobId) = {
    ...
    logger.debug("Node ({}): Queried for SummerConstructor, got {}", dag.getNodeName(node), opSummerConstructor: Any)

    opSummerConstructor match {
      case Some(cons) =>
        logger.debug("Node ({}): Using user supplied SummerConstructor: {}", dag.getNodeName(node), cons: Any)
        cons.get
  ...
  private[this] final def legacyBuilder(storm: Storm, dag: Dag[Storm], node: StormNode, jobID: JobId) = {
    ...
    logger.info("[{}] cacheSize lowerbound: {}", nodeName, cacheSize.lowerBound: Any)
    ...
    if (cacheSize.lowerBound == 0) {
      ...
      logger.info("[{}] softMemoryFlush : {}", nodeName, softMemoryFlush.get: Any)

      val flushFrequency = storm.getOrElse(dag, node, DEFAULT_FLUSH_FREQUENCY)
      logger.info("[{}] maxWaiting: {}", nodeName, flushFrequency.get: Any)

      val useAsyncCache = storm.getOrElse(dag, node, DEFAULT_USE_ASYNC_CACHE)
      logger.info("[{}] useAsyncCache : {}", nodeName, useAsyncCache.get: Any)
      ...
      if (!useAsyncCache.get) {
        ...
        logger.info("[{}] asyncPoolSize : {}", nodeName, asyncPoolSize.get: Any)

        val valueCombinerCrushSize = storm.getOrElse(dag, node, DEFAULT_VALUE_COMBINER_CACHE_SIZE)
        logger.info("[{}] valueCombinerCrushSize : {}", nodeName, valueCombinerCrushSize.get: Any)
